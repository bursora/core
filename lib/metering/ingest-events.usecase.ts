 ...
 *   3. Persist the batch in a single repository call.
 *
 * Missing pricing rows surface as `UnknownPricingError` from the resolver.
 * The route handler renders that as a 400 `pricing_unknown` response so the
 * SDK author and the customer's ops see the gap; silently storing
 * cost_usd = 0 would hide real spend until the bill landed (see issue #915).
 */
...
export async function ingestEventsUseCase(input: IngestEventsInput): Promise<IngestSummary> {
    ...
    const rows: UsageEventRow[] = await Promise.all(
    ...
    // `inserted` is rows actually written. Retried `requestId`s dedup at the
    // unique index and are excluded, so the caller bills the bundle by real
    // writes, not the requested count (issue #1002).
    const inserted = await input.eventsRepo.insertBatch(rows);
