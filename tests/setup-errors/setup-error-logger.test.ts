...
import { setSetupErrorLoggerForTesting } from "@/lib/setup-errors/server";
...
// No-op delegate: these tests assert only that call sites forward through the
// seam, not that the recorder persists anything.
const recordingLogger = (): TrackingSetupErrorLogger =>
    new TrackingSetupErrorLogger({ log: async () => {} });
