...
/**
 * Returns a `status = ?` predicate for `'ok'` or `'blocked'`, or `undefined`
 * for `'both'` (no predicate). When the filter is omitted, defaults to `'ok'`
 * so existing callers keep their historical behavior.
 */
const statusFilter = (status: MeteringStatusFilter | undefined) => {
    const effective = status ?? "ok";
    if (effective === "both") return undefined;
    return eq(usageEvents.status, effective);
};

export const drizzleMeteringReadRepository = (db: Db): MeteringReadRepository => ({
    spendSeries: async (query: SpendSeriesQuery): Promise<readonly SeriesPoint[]> => {
        const tagCol = facetColumn(query.facet);
        const bucket = sql<
            Date | string
        >`to_timestamp(floor(extract(epoch from ${usageEvents.ts}) / ${query.bucketSeconds}) * ${query.bucketSeconds})`;
    ...
    listDistinctValuesBulk: async (
        ...
        const statusPredicate = statusFilter(query.status);
        ...
        const results = await Promise.all(
            query.scopes.map(async (scope) => {
                ...
                if (statusPredicate !== undefined) conditions.push(statusPredicate);
                const rows = await db
    ...
    getLastUsageEventAt: async (query: LastUsageEventAtQuery): Promise<Date | null> => {
        const [row] = await db
            ...
            .from(usageEvents)
            .where(
                and(eq(usageEvents.workspaceId, query.workspaceId), eq(usageEvents.status, "ok")),
            );
    ...
    cumulativeSpendDaily: async (query: CumulativeSpendDailyQuery): Promise<readonly number[]> => {
        ...
        for (const r of rows) {
            const ts = r.bucket instanceof Date ? r.bucket.getTime() : new Date(r.bucket).getTime();
